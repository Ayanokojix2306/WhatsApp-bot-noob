...
connectDB().catch(error => {
    console.error('lol MongoDB connection error:', error);
});
...
async function connectToWhatsApp() {
    ...
    const { state, saveCreds } = await getMongoAuthState(); // Now retrieve the auth state from MongoDB
...
app.get('/', (req, res) => {
    console.log('Current QR Code:', currentQrCode); // Check what this logs
    res.send(`
        <html>
            <body>
